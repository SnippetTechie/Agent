...
import { ChevronDown, Circle, CircleCheck, CircleX, Eye, EyeOff, LoaderCircle } from "lucide-react";
...
function StepRow({ step }: { step: AgentStep }) {
  ...
  return (
    <li className="relative animate-varma-rise pl-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
        <div className="min-w-0">
          <p
            className={`text-[13px] leading-tight transition-colors duration-300 ${step.status === "pending" ? "text-varma-text-dim" : "text-varma-text"}`}
          >
            {step.label}
          </p>
          {step.detail && <p className="text-[11px] leading-snug text-varma-text-dim">{step.detail}</p>}
        </div>
...
export function ActionCard({ turn }: { turn: AgentTurn }) {
  const { t } = useI18n();
